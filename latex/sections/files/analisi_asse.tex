...
$$ x_\theta = x_0 \cos \theta \hspace{1.5cm} y_theta = y_0 \sin \theta \hspace{1.5cm} r_theta = r_0 \sqrt{\frac{\nu_\theta}{\nu_0}}$$
...
I cerchi si possono vedere nella Figura \ref{gr:asse_rotazione}\\
...
Perciò andando a vedere il raggio stimato precedentemente come valore atteso della distanza, si può fare un fit a due parametri liberi con quattro punti per ottenere
...
Figura \ref{gr:asse_fit}. Questo rivela come la sorgente sia nel punto:
$$(0.0013 \pm 0.0003, 0.0079 \pm 0.0003)$$
\input{./graphs/asse_bidim}
...
è uguale a $\sqrt{(1+0.0013)^2+(0.0079)^2} = 1.0013$
e perciò l'unità nel sistema di riferimento precedente è data da:
$$u =\frac{R1}{1.0013}=14.1 \text{cm}$$
...
sorgente è spostata a destra di 0.018~cm e verso l'alto di 0.114~cm.\\
...
trigonometrica, sapendo la posizione della sorgente rispetto all'asse di rotazione, si trovano i valori riassunti nella Tabella \ref{tab:asse_correzioni}\footnote{Si noti
...
$$\theta' = 180 -\arctan\left(\frac{y_s-\sin\theta}{x_s+\cos\theta}\right) \hspace{3 cm} d' = \sqrt{(x_0+\cos\theta)^2+(y_0-\sin\theta)^2}$$
